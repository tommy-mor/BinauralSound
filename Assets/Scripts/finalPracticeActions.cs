...
public class finalPracticeActions : UdonSharpBehaviour
{
    ...
    private int[] trialSizeArray = new int[8] { 50, 100, 200, 400, 50, 100, 200, 400 };
        ...
        if (onPlanet.y < sphereRadius * .75)
        {
            ...
            else
            {
                ...
                if (checkResult.Length == 0)
                {
                    newObject = VRCInstantiate(spawnItem);
                    newObject.transform.position = this.transform.position + onPlanet;
                    newObject.GetComponent<testPin>().SetUnSpecial(count);
